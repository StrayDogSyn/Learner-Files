...
const getAITemplate = (subject, projectInterest) => {
  const templates = {
...
const detectSpam = (formData) => {
  ...
  if (formData.message.length < 10 && formData.message.includes('http')) spamScore += 5;
  if (formData.email.includes('@temp.com') || formData.email.includes('@test.com')) spamScore += 3;
...
const useAutoSave = (formData, formId = 'contact-form') => {
...
export const SmartContactForm = () => {
  ...
  const [showTranslationOffer, setShowTranslationOffer] = useState(false);
  ...
  // Trigger success actions
  const triggerSuccessActions = async (data) => {
    try {
